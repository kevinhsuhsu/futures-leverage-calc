  ...
  const open = () => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (m.event === 'authenticated') {
        ...
        console.warn('[ws] error', m.data);
        onStatus?.('error', m.data?.message || '');
      } else if (m.event === 'data' || m.event === 'snapshot') {
        const d = m.data || {};
        // trades 頻道：價格在 data.trades[] 陣列，取最後一筆成交
        const last = Array.isArray(d.trades) && d.trades.length ? d.trades[d.trades.length - 1] : null;
        const p = last?.price ?? d.price ?? d.lastPrice ?? d.close ?? d.closePrice;
